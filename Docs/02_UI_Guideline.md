...
完全隐藏系统光标，JS 渲染 SVG 光标跟随鼠标。
- **CSS**: `cursor:none!important` + 透明PNG兜底
- **JS**: `position:fixed` div，z-index:99999，`mousemove` 更新位置
- **Electron**: 主进程 `executeJavaScript` 注入 `cursor:none`
- 默认态: 绿色十字准星，可点击元素悬停: 绿色实心锁定点
